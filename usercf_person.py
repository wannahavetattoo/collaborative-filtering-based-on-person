...
def getAllUserRating(n=1, k=20, similarity=sim_pearson):
    if (n == 1):
        ...
        testdata = loadMovieLensTest(
   'ml-100k/u1.test')  # 加载测试集
        #print("测试集训练集加载结束\n")
        ...
        for line in loadfile(ratingfile):
            user, movie, rating, _ = line.split('::')
            # split the data by pivot
...
if __name__ == "__main__":
    ...
    '''
        ...
        print("%3d%19.3f%%%19.3f%%%17ss" % (k, rmse * 100, mae * 100, (datetime.datetime.now() - starttime).seconds))
         r = getAllUserRating(1, 10)
    print("%3s%20s%20s%20s" % ('K', "RMSE", "MAE", "耗时"))

    rmse = getRMSE(r)
    mae = getMAE(r)
    print("%3d%19.3f%%%19.3f%%%17ss" % (25, rmse * 100, mae * 100, (datetime.datetime.now() - starttime).seconds))
    '''
    #ratingfile = os.path.join('ml-1m', 'ratings.dat')

    print("%3s%20s%20s%20s" % ('K', "RMSE", "MAE", "耗时"))
    for k in [25, 50, 75, 100, 125, 150]:
        r = getAllUserRating(1, k)
        rmse = getRMSE(r)
        mae = getMAE(r)
        print("%3d%19.3f%%%19.3f%%%17ss" % (k, rmse * 100, mae * 100, (datetime.datetime.now() - starttime).seconds))
